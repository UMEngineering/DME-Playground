...
var umcoe = {
    init: function () {
        ...
        if ($(document.body).hasClass("portaltype-uom-pfiles-profile")) {
            $("#tabs").profiletabs({ labelledBy: "tabsLbl" });
          // Adds profile tabs and ARIA for tabs
        }

        $("div#contentWrap div.promoHolderProfiles span.profilesMichEng").fadeTo(3500,0.75).fadeOut(1500); 
      
        if (!$("html.rgba").length) {
            this.tableFallback();
        }
        this.setupVideoPortlet();
        this.setupVideoStrip();
    }, // end init
   
    setupVideoPortlet: function () {
        // setup video portlet - ticket #1291
        var $ytColorbox = $(".youtubeColorbox");

        // Loop over all the video portlets
        $ytColorbox.each(function (index) {
            var $this = $(this), // Current element.
                $temp = $this.children("div").detach(), // Remove contents and store for later use.
                $id = $this.attr("id");

            // Add html containers, set background image, add play button, re-insert content in title container.
            $this
                .append("<div class='youtube-div' id='div-" + $id + "'><div class='play-button-videoportlet'><div class='youtube-title'></div></div></div>")
                    .children("#div-" + $id)
                    .css("background", "url(http://i2.ytimg.com/vi/" + $id + "/0.jpg) center no-repeat")
                    .find(".youtube-title")
                        .html($temp);
    ...
    setupVideoStrip: function () {
        // Setup videostrip images - ticket 1251.
        $(".videoStrip .thumbnail").each(function (index) {
            var $this = $(this), // The current strip item.
                $img = $this.children("img"); // Image in current item.

            // Set item background to image source, add play button overlay, then remove child image.
            $this.css("background", "url(" + $img.attr("src") + ")")
                .append("<div class='play-button-videostrip'><span style='visibility: hidden;'>thumbnail image " + (index + 1) + "</span></div>");
            $img.remove();
        });
    },

    tableFallback: function () {
        // Alternate background on rows if the browser doesn't support CSS3.
        $(".altByRow tr").each(function (index) {
            if (index % 2 !== 0) {
        ...
        $(".altByCol tr").each(function (index) {
            if ($(this).children("th")) {
    ...
    setupSideNav: function () {
        ...
        // Adjust contact portlet if there are parent portlets on the page.
        var $navNeighbor = $sideNav.parents(".portletWrapper").next().children(),
            $contactPort = $("div.promoCol div.contactPortlet");
        // If the nav has siblings and the first one is not contact portlet and there's a contact portlet present.
        if ($navNeighbor.length && !$navNeighbor.hasClass("contactPortlet") && $contactPort.length) {
            // Add space below the nav.
            $sideNav.css("padding-bottom", "234px");
            // Pull the contact portlet up into that space.
            $contactPort.css("margin-top", "-234px");
        } 
