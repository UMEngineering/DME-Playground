...
/* - ++resource++umich_js/calendarUtils.js - */
...
var calendarUtils = {
    ...
    processMonthJSONdata: function (data) {
        ...
        umcoe.trimString(".minDetails p b", 40);
        umcoe.trimString(".minDetails p span", 30);
        umcoe.trimString(".notTrimmed", 10000);        
    ...
    processListJSONdata: function (data) {
        ...
        //Trim the length of the short description.
        umcoe.trimString(".detailsCopy", 370);        

        //Fixing weird character encoding/decoding issue with this silly fix
        umcoe.trimString(".notTrimmed", 10000); 
        //Reload the addthis javascript so it can update the newly rendered social buttons
        umcoe.reloadAddThis();

        // If the events array is empty (no events to show).
        if (!data.events.length) {
           // Output message to user.
          $(".eventListing").append("<div class='item noResults'><div class='itemShadow'>" +
          "<div class='details'>There are no events on the calendar that match your" +
          " selected criteria.</div></div></div>");
        }
    ...
    updateViewLinks: function (m, d, y) {
        ...
        $("#monthView, #weekView, #listView").each(function () {
			...
			// Check if the link is current month
			//+++++++++++++++++++++++++++++++++++++++++++++++
			/*var today = new Date();
			var write_month = m;
			var write_day = d;
			var write_year = y;
			if (($(this).attr("id") == "monthView" || $(this).attr("id") == "listView") && m != today.getMonth()+1) {
				write_month = today.getMonth() + 1;
				write_day = 1;
				write_year = today.getYear();
			}
            $this.attr("href", strippedHref + "?date=" + write_year + "/" + write_month + "/" + write_day);*/
    ...
    drawMonthHeader: function (m, y) {
        ...
        $(".calendarNav span.currentMonth").text(month + " " + y).attr("rel", y + "/" + dispMon + "/1");
    ...
    drawWeekHeader: function (m, y, d) {
        ...
        var startDate = new Date(y, m, d),
            ...
            fdow = startDate.getDate() - startDate.getUTCDay(), //find first day of week
            prevYear = m < 1 && fdow < 8 ? y * 1 - 1 : y,
            nextYear = m * 1 + 1 > 11 ? y * 1 + 1 : y;
        ...
        nextYear = nextStartDate.getFullYear();
    ...
    drawDay: function (cell, date, active, link_days) {
        if(typeof(link_days)==='undefined') link_days=true;
        var curDate = Date();
        if (date == curDate) {
        ...
        if(link_days) {
          ...
          $("#cell" + cell + " .day").append("<h2><span class='dayLink'>" + date.getDate() + "</span></h2>");
        }
    ...
    drawWeek: function (m, y, dy) {
        ...
        for (i = 1; i <= 7; i++) {
            ...
            calendarUtils.drawDay(i, d, true, false);
            fdow += 1;
        }
